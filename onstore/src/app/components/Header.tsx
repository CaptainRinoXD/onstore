...
const Header: React.FC<HeaderProps> = ({
  ...
  const MenuItemH = ({
    ...
    <li className="relative group transition-all hover-to-show-link">
      <Link
        href={href}
        className={`${className} cursor-pointer hover:text-blue-300 active:text-blue-500 transition-all duration-300`}>
      ...
      <div className="flex space-x-4">
        {newestCollections.map((collection) => (
          <div
            ...
            <Link
              href={`/producttypes?collection=${collection._id}`}
    ...
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        const response = await fetch(
          `http://localhost:3002/api/product_search/search?query=${encodeURIComponent(
            query
          )}`
        ); // Use encodeURIComponent to ensure the query is URL-safe
        if (response.ok) {
  ...
  return (
    <header
      className={`fixed top-0 w-full flex justify-between items-center shadow-md p-2 z-50 pt-4 pb-4 ${
      ...
      <nav
        className={`flex-grow items-center justify-center md:flex ml-50percentage ${
        ...
        <ul className="flex space-x-6">
          ...
          <li
            className="relative group transition-all hover-to-show-link"
            onMouseEnter={fetchProductTypes}>
            <a
              href="/producttypes"
      ...
      <div
        className={`fixed top-0 left-0 h-full w-64 shadow-lg transform transition-transform duration-300 ease-in-out z-50 md:hidden ${
      ...
      <div className="relative flex items-center mr-2">
        <div className="flex items-center relative mr-4">
          ...
          <div
            ...
            className={`absolute z-10 mt-1 bg-white shadow-md max-h-60 overflow-y-auto w-full border rounded top-12 ${
              !isSearchVisible || searchResults.length === 0 ? 'hidden' : ''
            }`}>
        ...
        <div className="relative cursor-pointer mr-4">
          ...
          <span className="absolute top-[-10px right-[-10px] bg-yellow-500 text-black text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
            1
          </span>
        </div>
...
const UserModal: React.FC<{
  ...
  isAdmin: boolean;
  isEmmployee: boolean;
}> = ({ username, handleLogout, toggleUserModal, isAdmin, isEmmployee }) => {
  const router = useRouter();
  return (
    <div className="absolute top-16 right-5 bg-white shadow-md p-6 rounded-lg z-20 w-72 border border-gray-200">
      <div className="flex flex-col items-center space-y-4">
        ...
        <p className="text-sm italic text-gray-500 text-center">
          Clothes aren't going to change the world. <br /> The women who wear
          them will.
        </p>
